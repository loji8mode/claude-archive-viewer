        ...
        if thought_block:
            ...
            if len(inner_blocks) <= 1:
                ...
                for ib in inner_blocks:
                    if ib["type"] == "code":
                        blocks.append({
                            "type": "thought_code", 
                            "title": "Код в роздумах (" + ib["title"].replace("Код (", "").replace(")", "") + ")", 
                            "content": ib["content"]
                        })
                    elif ib["type"] == "text":
            ...
            if code_match:
                ...
                if lang in ['html', 'htm', 'svg', 'xml']:
                    ...
                    blocks.append({
                        "type": "code",
                        "title": "Код (" + (lang if lang else 'text') + ")",
                        "content": code
                    })
            else:
...
def build_html_viewer(json_file_path, output_html_path):
    if not os.path.exists(json_file_path):
        print("Помилка: Файл " + json_file_path + " не знайдено!")
        return
    ...
    for conv in conversations:
        chat_data = {
            "name": conv.get("name") or "Діалог (" + conv.get('uuid', '')[:5] + ")",
            "messages": []
        }
                    ...
                    if b_type == 'text':
                        ...
                        if 'widget_code' in tool_input:
                            ...
                            msg_entry["blocks"].append({
                                "type": "artifact", 
                                "title": "Інтерактивний віджет (" + tool_name + ")", 
                                "content": style_inject + tool_input['widget_code']
                            })
                        else:
                            msg_entry["blocks"].append({
                                "type": "tool_call",
                                "title": "Виклик інструменту: " + tool_name,
...
<body>
    ...
    <div id="sidebar">
        <div class="sidebar-title">Ваші діалоги Claude</div>
        <div id="chatList"></div>
    </div>
    ...
    <div id="chat-container">
        <div id="chat-header">
            ...
            <button id="global-toggle-btn" onclick="toggleAllThoughts()">👁️ Показати роздуми та інструменти</button>
        </div>
        <div id="messages">
            <div class="placeholder">Оберіть діалог з панелі ліворуч для перегляду повної історії.</div>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleAllThoughts() {
            ...
            btn.textContent = globalThoughtsOpen ? '🙈 Сховати роздуми та інструменти' : '👁️ Показати роздуми та інструменти';
        }
        ...
        function showChat(index) {
            ...
            document.getElementById('global-toggle-btn').textContent = '👁️ Показати роздуми та інструменти';
            ...
            if (messages.length === 0) {
                container.innerHTML = '<div class="placeholder">Цей діалог порожній.</div>';
                return;
            }
            ...
            messages.forEach((msg, msgIdx) => {
                ...
                senderDiv.textContent = msg.sender === 'human' ? 'Ви' : 'Claude';
                ...
                msg.blocks.forEach((block, blockIdx) => {
                    if (block.type === 'text') {
                        ...
                        const { wrap, contentDiv } = makeCollapsible(
                            'Хід думок Claude (' + block.content.length.toLocaleString('uk-UA') + ' символів)',
                            '🧠'
                        );
                        ...
                        pBtn.textContent = 'Попередній перегляд';
                        
                        const cBtn = document.createElement('button');
                        cBtn.className = 'art-tab-btn';
                        cBtn.textContent = 'Код';
                        
                        const dlBtn = document.createElement('button');
                        dlBtn.className = 'art-download-btn';
                        dlBtn.textContent = 'Скачати...';
                        ...
                        const { wrap, contentDiv } = makeCollapsible(
                            'Результат виконання (' + block.content.length.toLocaleString('uk-UA') + ' символів)',
                            '📋'
                        );
                        contentDiv.classList.add('thought-text');
                        contentDiv.textContent = '[Системний лог виконання]:\\n' + block.content;
                        bodyDiv.appendChild(wrap);
                    }
                });
...
""".replace('__DATA_BUFER__', safe_json_data)
    ...
    print("Успішно згенеровано! Файл збережено: " + output_html_path)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Claude Archive Smart Viewer — конвертація експорту діалогів у зручний HTML.")
    parser.add_argument('-i', '--input', default='conversations.json', help="Шлях до вхідного файлу JSON")
    parser.add_argument('-o', '--output', default='viewer.html', help="Шлях до вихідного файлу HTML")
    
    args = parser.parse_args()

    if not os.path.exists(args.input):
        print(f"Помилка: Вхідний файл '{args.input}' не знайдено.")
        sys.exit(1)

    try:
        build_html_viewer(args.input, args.output)
    except json.JSONDecodeError:
        print(f"Помилка: Файл '{args.input}' містить некоректний JSON.")
        sys.exit(1)
    except Exception as e:
        print(f"Відбулася непередбачувана помилка: {e}")
        sys.exit(1)